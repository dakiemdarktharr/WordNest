...
1. Mở [WordNest 0.3.2 trên GitHub Releases](https://github.com/dakiemdarktharr/WordNest/releases/tag/v0.3.2).
2. Mở phần **Assets** nếu danh sách đang thu gọn.
3. Chọn đúng file theo máy:
   - `WordNest-Setup-0.3.2-x64.exe` cho Windows 10/11 x64.
   - `WordNest-0.3.2-x64.dmg` cho Mac Intel.
   - `WordNest-0.3.2-arm64.dmg` cho Mac Apple Silicon.
   - File `.zip` tương ứng là lựa chọn thay thế cho macOS.
4. Chờ tải xong, rồi mở thư mục Downloads / Tải xuống.

[Tải bộ cài từ GitHub Release v0.3.2](https://github.com/dakiemdarktharr/WordNest/releases/tag/v0.3.2)
...
1. Nhấp đúp vào `WordNest-Setup-0.3.2-x64.exe`.
...
5. Bản 0.3.2 được ký ad-hoc, chưa có Developer ID/notarization của Apple. Nếu thông báo nói không xác minh được nhà phát triển và bạn tin tưởng nguồn, vào **System Settings → Privacy & Security → Open Anyway**.

Nếu đang gặp “WordNest is damaged” ở 0.2.2: đóng app, tải DMG 0.3.2 đúng loại chip, kéo vào Applications và chọn **Replace** để thay app cũ. Eject DMG rồi mở WordNest từ Applications. Không xóa thư mục dữ liệu WordNest trong Library; thao tác thay app giữ hồ sơ học. Nếu vẫn báo “damaged”, giữ nguyên thông báo để báo lỗi, không tắt Gatekeeper hoặc chạy lệnh xóa quarantine hàng loạt.

Không cần cài Node.js, chạy terminal, mở localhost hoặc giữ trình duyệt hoạt động.

## 4. Nhập TXT và bắt đầu học

1. Nhận file TXT từ giáo viên, hoặc tải [file mẫu](https://github.com/dakiemdarktharr/WordNest/releases/download/v0.3.2/wordnest-demo.txt) trong Assets của release.
...
Màn hình kết quả phân biệt **đã luyện đúng 100%** với **điểm lần đầu**; câu từng sai vẫn được hẹn ôn sớm. Luyện tập và luyện gõ không giới hạn thời gian. Hãy dùng app 0.3.2 trở lên khi khôi phục bản sao lưu có lượt luyện mới. Tùy chọn sáng/tối không nằm trong bản sao lưu thư viện.
